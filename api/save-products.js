...
export default async function handler(req, res) {
  ...
  if (!Array.isArray(products)) return res.status(400).json({ error: 'Payload invalide: "products" doit être un tableau' });
  ...
  try {
    ...
    else if (getResp.status !== 404) { return res.status(getResp.status).json(await safeJson(getResp)); }
