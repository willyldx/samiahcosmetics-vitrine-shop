...
// VERSION CORRIGÉE (Suggestion 2.1) : gère les DEUX tables (ancienne + nouvelle)
...
export default async function handler(req, res) {
  try {
    ...
    for (const p of products) {
      ...
      // 1. Préparer le produit principal
      const productRow = {
        id: p.id,
        title: p.title ?? '',
      ...
      // 3. Supprimer les anciennes images de la table 'product_images'
      await supabaseFetch(`product_images?product_id=eq.${encodeURIComponent(p.id)}`, {
        method: 'DELETE',
        headers: { 'Prefer': 'return=minimal' }
      });

      // 4. Insérer les nouvelles images dans la table 'product_images'
